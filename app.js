    ...
    builder.defineStreamHandler(async function(args) {
        if (args.type === 'tv' && args.id.startsWith('sportslive:')) {
            ...
            if (eventGroup.links && eventGroup.links.length > 0) {
                ...
                const enabledProviders = userConfig.enabledProviders?.length > 0
                    ? userConfig.enabledProviders
                    : AVAILABLE_STREAM_PROVIDERS.map(p => p.id);

                let optionCounter = 1;

                for (const link of eventGroup.links) {
                    ...
                    for (const providerId in streamProviders) {
                        if (enabledProviders.includes(providerId)) {
                            ...
                            try {
                                ...
                                console.error(`[ADDON] Error al descifrar por ${providerId} para ${eventGroup.title} (enlace ${link}):`, error.message);
                            }
                        }
                    }
                }
...
.then(([_, allEventsData]) => { 
    ...
    builder = addonBuilder({
        ...
        version: '1.0.0',
        name: 'Sports Live',
        description: 'Eventos deportivos en vivo',
